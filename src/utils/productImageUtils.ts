...
export const getHeadphoneImage = (name: string, brand: string): string => {
  ...
  if (name.toLowerCase().includes('quietcomfort') && brand.toLowerCase() === 'bose') {
    return '/lovable-uploads/7be48add-b36a-4617-8856-47352e844bae.png';
  }
  
  // Apple headphones
  if (brand === "Apple" && !name.toLowerCase().includes("airpod")) {
  ...
  // Sports headphones
  if (name.toLowerCase().includes("sports") || (name.toLowerCase().includes("sport") && name.toLowerCase().includes("headphones"))) {
...
export const getCategoryImage = (category: string, brand?: string, name?: string): string => {
  ...
  if (category === 'TVs') {
    if (brand === 'Samsung' && name && name.toLowerCase().includes('oled')) {
  ...
  if (category === 'PC Accessories' && name && name.toLowerCase().includes('keyboard')) {
    return '/lovable-uploads/3f391803-44c1-4437-a246-5070b31d60a5.png';
  }
  
  // Handle specific brand cases
  if (brand) {
    if (brand === 'Nintendo' || (name && (name.toLowerCase().includes('nintendo') || name.toLowerCase().includes('switch')))) {
      return '/lovable-uploads/54b67814-dd27-4a46-ac69-4beaf7bd7851.png';
    }
    
    if (brand === 'Google' && name && name.toLowerCase().includes('chromebook')) {
