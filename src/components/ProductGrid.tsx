...
const ProductGrid = ({
  ...
  return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      // Apply new image URLs based on product details - Add null checks to prevent undefined errors
      if (product.brand && product.name && product.brand.toLowerCase() === 'google' && product.name.toLowerCase().includes('chromebook')) {
        imageUrl = '/lovable-uploads/6b47797a-7950-4e45-9ec1-b49920bedce3.png';
      } else if ((product.brand && product.brand.toLowerCase() === 'xbox') || 
                (product.brand && product.category && product.brand.toLowerCase() === 'microsoft' && product.category.toLowerCase() === 'gaming consoles') ||
                (product.name && product.name.toLowerCase().includes('xbox'))) {
        imageUrl = '/lovable-uploads/4b5ba4b7-1d75-4c44-aa6c-c1d6e0d028c4.png';
      } else if ((product.name && product.name.toLowerCase() === 'playstation 5') || 
                (product.brand && product.name && product.brand.toLowerCase() === 'playstation' && !product.name.toLowerCase().includes('digital')) ||
                (product.name && product.name.toLowerCase().includes('playstation 5 digital')) || 
                (product.name && product.name.toLowerCase().includes('playstation') && product.name.toLowerCase().includes('digital'))) {
        imageUrl = '/lovable-uploads/245080f6-be80-47c6-b153-72bc982b50ac.png';
      } else if (product.name && product.name.toLowerCase().includes('battlefield')) {
        imageUrl = '/lovable-uploads/ab3d21b8-041b-4137-865c-22fe07795d75.png';
      } else if ((product.brand && product.brand.toLowerCase() === 'nintendo') || 
                (product.name && (product.name.toLowerCase().includes('nintendo') || product.name.toLowerCase().includes('switch')))) {
        imageUrl = '/lovable-uploads/54b67814-dd27-4a46-ac69-4beaf7bd7851.png';
      } else if (product.brand && product.category && product.brand.toLowerCase() === 'xiaomi' && product.category.toLowerCase() === 'smartphones') {
        imageUrl = '/lovable-uploads/19334ecf-42e6-4db9-8443-15db93e88166.png';
      } else if ((product.name && product.name.toLowerCase().includes('speaker')) || 
                (product.category && product.category.toLowerCase().includes('speaker'))) {
        imageUrl = '/lovable-uploads/4d754bb4-c77a-436a-8470-ef066e888a5d.png';
      } else if ((product.name && product.name.toLowerCase().includes('microphone')) || 
                (product.category && product.category.toLowerCase().includes('microphone'))) {
        imageUrl = '/lovable-uploads/55a7c5a8-4ffa-4448-8ada-7591813f3755.png';
      } else if (product.name && product.brand && product.name.toLowerCase().includes('quietcomfort') && product.brand.toLowerCase() === 'bose') {
        imageUrl = '/lovable-uploads/7be48add-b36a-4617-8856-47352e844bae.png';
      } else if (product.name && product.name.toLowerCase().includes('death stranding')) {
        imageUrl = '/lovable-uploads/e61d09d1-fb3f-4e38-aaca-2342513b89de.png';
      } else if (product.name && product.name.toLowerCase().includes('elden ring')) {
        imageUrl = 'https://cdn.cloudflare.steamstatic.com/steam/apps/1245620/header.jpg';
      } else if (product.name && product.name.toLowerCase().includes('call of duty')) {
        imageUrl = '/lovable-uploads/2f5f9ee3-73a7-48e2-b97a-5de770162a36.png';
      } else if (product.name && product.name.toLowerCase().includes('rainbow six')) {
        imageUrl = '/lovable-uploads/2b732385-bcb9-459e-981e-bb57c1860769.png';
      } else if (product.name && product.name.toLowerCase().includes('cyberpunk 2077')) {
        imageUrl = '/lovable-uploads/d0b5f6e9-d8a7-4e6d-92d9-0981cb533be3.png';
      } else if (product.name && product.name.toLowerCase().includes('horizon kings') && product.name.toLowerCase().includes('simulation')) {
        imageUrl = '/lovable-uploads/f58b103e-1e2f-4e40-92bd-5ceee55670d4.png';
      } else if (product.name && product.name.toLowerCase().includes('world of warriors') && product.name.toLowerCase().includes('racing')) {
        imageUrl = '/lovable-uploads/cf30cef5-878e-4911-b265-6fadc46cd9b1.png';
      } else if (product.name && product.name.toLowerCase().includes('ultimate fantasy') && product.name.toLowerCase().includes('racing')) {
        ...
        if (product.name && product.name.toLowerCase().includes('interactive panel')) {
          imageUrl = '/lovable-uploads/83220acc-b41f-488f-996c-70c790349093.png';
        } else if (product.name && product.name.toLowerCase().includes('smart digital display')) {
          imageUrl = '/lovable-uploads/f4d18f61-e011-41e1-8945-0862b8e9cb22.png';
        } else if (product.name && product.name.toLowerCase().includes('digital signage')) {
      ...
      return <div key={product.id} onClick={() => handleProductClick(product)} className="bg-card rounded-lg shadow-md overflow-hidden cursor-pointer hover:shadow-lg transition-shadow">
            <div className="relative">
              <img src={imageUrl} alt={product.name || 'Product'} className="product-image w-full h-48 object-contain" />
            ...
            <div className="p-4">
              <h3 className="text-lg font-semibold mb-1 line-clamp-1">{formatText(product.name || 'Unnamed Product')}</h3>
